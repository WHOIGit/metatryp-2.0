...
def _split_label(label):
        if not is_modX(label):
            raise Exception('Cannot split a non-modX label: %s' % label)
        if len(label) == 1:
            return (label, )
        else:
...
def isoforms(sequence, **kwargs):
    ...
    def main(group): # index of the residue (capital letter) in `group`
        ...
        if len(temp) != 1: raise Exception('Invalid group: %s' % group)
    ...
    if format_ == 'split': return product(*states)
        ...
        raise Exception('Unsupported value of "format": {}'.format(format_))
